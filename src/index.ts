...
class CloudAmazonSellerMCPServer {
    ...
    async setupRedis() {
        if (process.env.REDIS_URL) {
            try {
                ...
                console.error('Redis connection failed, using in-memory storage:', (error as Error).message);
            }
        } else {
    ...
    private async storeTokens(userId: string, tokens: UserTokens): Promise<void> {
        if (this.redis) {
            try {
                await this.redis.setEx(
                    `amazon_tokens:${userId}`,
                    3600 * 24 * 7,
    ...
    private async getTokens(userId: string): Promise<UserTokens | null> {
        if (this.redis) {
            try {
    ...
    private async checkAuthentication(userId: string): Promise<boolean> {
    ...
    private async initiateUserAuth(userId: string, region: string): Promise<string> {
        const clientId = process.env.AMAZON_CLIENT_ID;
        const redirectUri = process.env.OAUTH_REDIRECT_URI;
    ...
    private async makeApiRequest(userId: string, endpoint: string, method: 'GET' | 'POST' | 'PUT' | 'DELETE' = 'GET', body?: any): Promise<any> {
        const tokens = await this.getTokens(userId);
    ...
    private async getSellerAccount(userId: string): Promise<any> {
        try {
    ...
    private setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InternalError, `Error executing ${name}: ${(error as Error).message}`);
            }
        });
    ...
    private async handleAuthenticate(args: any) {
        try {
            ...
            return { content: [{ type: 'text', text: `Authentication setup failed: ${(error as Error).message}` }], isError: true };
        }
    }

    private async handleStatus(userId: string) {
        if (!userId) {
        ...
        if (await this.checkAuthentication(userId)) {
            try {
                ...
                return { content: [{ type: 'text', text: `Connected but unable to fetch details: ${(error as Error).message}` }] };
            }
        }
        return { content: [{ type: 'text', text: '**Not connected to Amazon Seller**\n\nRun amazon_authenticate to get started.' }] };
    }

    private async handleAmazonOperation(operation: string, args: any) {
        switch (operation) {
    ...
    private async listOrders(args: any) {
        ...
        if (created_after) params.push(`CreatedAfter=${created_after}`);
        if (created_before) params.push(`CreatedBefore=${created_before}`);
        if (order_statuses) params.push(`OrderStatuses=${order_statuses.join(',')}`);
        if (fulfillment_channels) params.push(`FulfillmentChannels=${fulfillment_channels.join(',')}`);
        if (max_results) params.push(`MaxResultsPerPage=${max_results}`);
    ...
    private async getOrder(args: any) {
        ...
        if (include_items) {
            ...
            } catch { info.push('Items: Unable to fetch items'); }
        }
        return { content: [{ type: 'text', text: info.join('\n') }] };
    }

    private async listInventory(args: any) {
        ...
        if (marketplace_ids) endpoint += `&marketplaceIds=${marketplace_ids.join(',')}`;
        if (seller_skus && seller_skus.length > 0) endpoint += `&sellerSkus=${seller_skus.join(',')}`;
        const response = await this.makeApiRequest(args.user_id, endpoint);
        const inventories = response.payload?.inventorySummaries || [];
        const inventoryList = inventories.slice(0, max_results).map((item: any) => {
    ...
    private async getReports(args: any) {
        ...
        const params = [];
        if (report_types) params.push(`reportTypes=${report_types.join(',')}`);
        if (processing_statuses) params.push(`processingStatuses=${processing_statuses.join(',')}`);
        if (marketplace_ids) params.push(`marketplaceIds=${marketplace_ids.join(',')}`);
        if (created_since) params.push(`createdSince=${created_since}`);
        if (max_results) params.push(`pageSize=${max_results}`);
    ...
    private async createReport(args: any) {
        ...
        const reportRequest: any = { reportType: report_type, marketplaceIds: marketplace_ids };
        if (data_start_time) reportRequest.dataStartTime = data_start_time;
        if (data_end_time) reportRequest.dataEndTime = data_end_time;
    ...
    private async getFinances(args: any) {
        ...
        if (posted_after) endpoint += `&PostedAfter=${posted_after}`;
        if (posted_before) endpoint += `&PostedBefore=${posted_before}`;
        const response = await this.makeApiRequest(args.user_id, endpoint);
        const eventGroups = response.payload?.FinancialEventGroupList || [];
        const financesList = eventGroups.map((group: any) => {
    ...
    private async confirmShipment(args: any) {
