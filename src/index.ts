...
class CloudAmazonSellerMCPServer {
            ...
            try {
                ...
                if (tokenResponse) {
                    const userTokens: UserTokens = {
                        ...
                        region: (state as string).split('_')[1] || 'us-east-1',
                    };
    ...
    private async handleAuthenticate(args: any) {
        try {
            ...
            return {
                content: [{
                    type: 'text',
                    text: `Authentication setup failed: ${(error as Error).message}`
                }],
                isError: true
            };
        }
    ...
    private async updateInventory(args: any) {
        ...
        try {
            ...
            return {
                content: [{
                    type: 'text',
                    text: `Inventory update failed: ${(error as Error).message}`
                }],
                isError: true
            };
        }
    ...
    private async confirmShipment(args: any) {
        ...
        await this.makeApiRequest(
            args.user_id, 
            `/orders/v0/orders/${order_id}/shipment`, 
            'POST', 
            shipmentData
        );
